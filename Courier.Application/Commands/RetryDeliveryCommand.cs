...
public class RetryDeliveryCommandHandler : IRequestHandler<RetryDeliveryCommand, Result<ShipmentDto>>
{
    ...
    public async Task<Result<ShipmentDto>> Handle(RetryDeliveryCommand request, CancellationToken cancellationToken)
    {
        ...
        var result = shipment.PrepareForRetry(request.NewAddress, request.OperatorUsername);
        if (result.IsFailure)
