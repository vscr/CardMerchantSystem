...
public record PickUpShipmentCommand(Guid ShipmentId, string OperatorUsername) : IRequest<Result<ShipmentDto>>;

public class PickUpShipmentCommandHandler : IRequestHandler<PickUpShipmentCommand, Result<ShipmentDto>>
{
    ...
    public async Task<Result<ShipmentDto>> Handle(PickUpShipmentCommand request, CancellationToken cancellationToken)
    {
        ...
        var result = shipment.MarkAsPickedUp(request.OperatorUsername);
        if (result.IsFailure)
