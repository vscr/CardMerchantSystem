...
public record GetPendingDeliveriesQuery() : IRequest<IReadOnlyList<ShipmentDto>>;

public class GetPendingDeliveriesQueryHandler : IRequestHandler<GetPendingDeliveriesQuery, IReadOnlyList<ShipmentDto>>
{
    private readonly IShipmentRepository _shipmentRepository;
    private readonly ICourierCompanyRepository _companyRepository;

    public GetPendingDeliveriesQueryHandler(
    ...
    public async Task<IReadOnlyList<ShipmentDto>> Handle(GetPendingDeliveriesQuery request, CancellationToken cancellationToken)
    {
        var shipments = await _shipmentRepository.GetPendingDeliveriesAsync(cancellationToken);
