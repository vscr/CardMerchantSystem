...
public class GetSwitchMessagesByDateQueryHandler
    : IRequestHandler<GetSwitchMessagesByDateQuery, IReadOnlyList<SwitchMessageDto>>
{
    private readonly ISwitchMessageRepository _repository;

    public GetSwitchMessagesByDateQueryHandler(ISwitchMessageRepository repository)
    {
        _repository = repository;
    }

    public async Task<IReadOnlyList<SwitchMessageDto>> Handle(
        GetSwitchMessagesByDateQuery request,
        CancellationToken cancellationToken)
    {
        var messages = await _repository.GetByDateRangeAsync(request.StartDate, request.EndDate, cancellationToken);
