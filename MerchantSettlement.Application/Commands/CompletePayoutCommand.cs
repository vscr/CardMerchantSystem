...
public record CompletePayoutCommand(Guid PayoutId, string BankReferenceNumber, string OperatorUsername) : IRequest<Result<MerchantPayoutDto>>;

public class CompletePayoutCommandHandler : IRequestHandler<CompletePayoutCommand, Result<MerchantPayoutDto>>
{
    private readonly IMerchantPayoutRepository _repository;

    public CompletePayoutCommandHandler(IMerchantPayoutRepository repository)
    {
        _repository = repository;
    }

    public async Task<Result<MerchantPayoutDto>> Handle(CompletePayoutCommand request, CancellationToken cancellationToken)
    {
        ...
        // Önce işleme al
        var processResult = payout.StartProcessing(request.OperatorUsername);
        if (processResult.IsFailure)
            return Result.Failure<MerchantPayoutDto>(processResult.Error);

        // Sonra öde
        var paidResult = payout.MarkAsPaid(request.BankReferenceNumber, request.OperatorUsername);
        if (paidResult.IsFailure)
            return Result.Failure<MerchantPayoutDto>(paidResult.Error);
