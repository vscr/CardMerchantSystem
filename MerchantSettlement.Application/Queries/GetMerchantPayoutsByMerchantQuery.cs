...
public record GetMerchantPayoutsByMerchantQuery(string MerchantId) : IRequest<IReadOnlyList<MerchantPayoutDto>>;

public class GetMerchantPayoutsByMerchantQueryHandler : IRequestHandler<GetMerchantPayoutsByMerchantQuery, IReadOnlyList<MerchantPayoutDto>>
{
    private readonly IMerchantPayoutRepository _repository;

    public GetMerchantPayoutsByMerchantQueryHandler(IMerchantPayoutRepository repository)
    {
        _repository = repository;
    }

    public async Task<IReadOnlyList<MerchantPayoutDto>> Handle(GetMerchantPayoutsByMerchantQuery request, CancellationToken cancellationToken)
    {
        var payouts = await _repository.GetByMerchantIdAsync(request.MerchantId, cancellationToken);
