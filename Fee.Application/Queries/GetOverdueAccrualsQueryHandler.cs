...
public class GetOverdueAccrualsQueryHandler
    : IRequestHandler<GetOverdueAccrualsQuery, IReadOnlyList<FeeAccrualDto>>
{
    private readonly IFeeAccrualRepository _repository;

    public GetOverdueAccrualsQueryHandler(IFeeAccrualRepository repository)
    {
        _repository = repository;
    }

    public async Task<IReadOnlyList<FeeAccrualDto>> Handle(
        GetOverdueAccrualsQuery request,
        CancellationToken cancellationToken)
    {
        var accruals = await _repository.GetOverdueAsync(cancellationToken);
