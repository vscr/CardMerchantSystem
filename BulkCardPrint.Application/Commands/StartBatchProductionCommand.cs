...
public record StartBatchProductionCommand(Guid BatchId, string OperatorUsername) : IRequest<Result<PrintBatchDto>>;

public class StartBatchProductionCommandHandler : IRequestHandler<StartBatchProductionCommand, Result<PrintBatchDto>>
{
    private readonly IPrintBatchRepository _batchRepository;
    private readonly IPrintVendorRepository _vendorRepository;

    public StartBatchProductionCommandHandler(
    ...
    public async Task<Result<PrintBatchDto>> Handle(StartBatchProductionCommand request, CancellationToken cancellationToken)
    {
        ...
        var startResult = batch.StartProduction(request.OperatorUsername);
        if (startResult.IsFailure)
            return Result.Failure<PrintBatchDto>(startResult.Error);
