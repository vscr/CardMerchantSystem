...
public record GetCompletedWorkOrdersQuery() : IRequest<IReadOnlyList<WorkOrderItemDto>>;

public class GetCompletedWorkOrdersQueryHandler : IRequestHandler<GetCompletedWorkOrdersQuery, IReadOnlyList<WorkOrderItemDto>>
{
    private readonly IWorkOrderItemRepository _itemRepository;
    private readonly IWorkOrderTypeRepository _typeRepository;

    public GetCompletedWorkOrdersQueryHandler(IWorkOrderItemRepository itemRepository, IWorkOrderTypeRepository typeRepository)
    {
        _itemRepository = itemRepository;
        _typeRepository = typeRepository;
    }

    public async Task<IReadOnlyList<WorkOrderItemDto>> Handle(GetCompletedWorkOrdersQuery request, CancellationToken cancellationToken)
    {
        var items = await _itemRepository.GetCompletedOrdersAsync(cancellationToken);
