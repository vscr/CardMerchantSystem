...
public record GetGeneratedReportsByStatusQuery(int StatusId) : IRequest<IReadOnlyList<GeneratedReportDto>>;

public class GetGeneratedReportsByStatusQueryHandler : IRequestHandler<GetGeneratedReportsByStatusQuery, IReadOnlyList<GeneratedReportDto>>
{
    ...
    public GetGeneratedReportsByStatusQueryHandler(
    ...
    public async Task<IReadOnlyList<GeneratedReportDto>> Handle(GetGeneratedReportsByStatusQuery request, CancellationToken cancellationToken)
    {
        var status = Enumeration.FromId<ReportStatus>(request.StatusId);
        if (status is null)
            return new List<GeneratedReportDto>();
